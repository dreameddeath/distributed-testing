...
public class AnnotationProcessorTestingWrapper {
    ...
    public Result run(String path) throws Exception {
        ...
        if (effectivePath.startsWith("classpath:")) {
            String pathInClassPath = effectivePath.substring("classpath:".length());
            if (pathInClassPath.startsWith("/")) {
                pathInClassPath = pathInClassPath.substring(1);
            }
            URL resource = this.getClass().getClassLoader().getResource(pathInClassPath);
            Preconditions.checkNotNull(resource, "Cannot find resource " + path);
            effectivePath = resource.getPath();
        }
        Iterable<JavaFileObject> files = getSourceFiles(effectivePath);
        return run(files);
    }
    ...
    public static class Result {
        ...
        public Field getField(String className, String fieldName) throws ClassNotFoundException, NoSuchFieldException {
            return outputClassLoader.loadClass(className).getField(fieldName);
        }
        ...
        public void updateSystemClassLoader() {
