...
import com.dreameddeath.common.lang.utils.ClassUtils;
...
public class PackageInfo extends AnnotatedInfo {
    ...
    static PackageInfo getPackageInfo(Package aPackage) {
    ...
    static PackageInfo getPackageInfo(PackageElement packageElement) {
        synchronized (packageElementToInfoMap) {
    ...
    static PackageInfo getPackageInfo(TypeElement element) {
    ...
    private static Package getPackage(PackageElement elt) {
        return ClassUtils.getEffectiveClassLoader().getDefinedPackage(elt.getQualifiedName().toString());
    }
