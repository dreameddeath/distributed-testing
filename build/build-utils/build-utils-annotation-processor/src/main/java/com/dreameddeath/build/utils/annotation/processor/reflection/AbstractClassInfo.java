...
public abstract class AbstractClassInfo extends AnnotatedInfo {
    ...
    private static String getClassName(TypeElement element) {
        ...
        while (currElement.getEnclosingElement() != null) {
            ...
            if (currElement instanceof TypeElement) {
                stringBuilder.insert(0, currElement.getSimpleName() + "$");
            } else if (currElement instanceof PackageElement) {
                if (!"".equals(currElement.getSimpleName().toString())) {
                    stringBuilder.insert(0, ((PackageElement) currElement).getQualifiedName() + ".");
                }
            }
        }
    ...
    private void init() {
        Preconditions.checkArgument(typeElement != null || clazz != null, "Either a clazz or a typeElement should be provided");
        if (typeElement != null) {
            synchronized (typeElementToInfoMap) {
        ...
        this.isBaseType =
                AbstractClassInfo.getClassInfo(String.class).isAssignableFrom(this)
                        ...
                        || AbstractClassInfo.getClassInfo(LocalDate.class).isAssignableFrom(this)
                        || AbstractClassInfo.getClassInfo(LocalDateTime.class).isAssignableFrom(this)
                        || AbstractClassInfo.getClassInfo(LocalTime.class).isAssignableFrom(this)
